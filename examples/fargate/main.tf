...
# ECS Cluster Configuration with CloudMap Integration
          ...
          command = [
            ...
            <<-EOT
              while true; do
                ...
                curl -s -f http://hello-webapp.${local.base_name}-internal:8080 > /dev/null && echo 'http://hello-webapp.${local.base_name}-internal:8080 = ok' || echo 'http://hello-webapp.${local.base_name}-internal:8080 = fail'
...
output "ecs_cluster_fargate" {
  description = "ECS Cluster Fargate"
